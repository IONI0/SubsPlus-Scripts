...
def generate_chapters(offset_list, file_duration, out_path):
    ...
    if snap_beginning:
        ...
        outfile.write(f"CHAPTER01NAME=Prologue\n")
        outfile.write(f"CHAPTER02={get_timestamp(offset_list[0])}\n")
        outfile.write("CHAPTER02NAME=Opening\n")
        outfile.write(f"CHAPTER03={get_timestamp(offset_list[1])}\n")
...
def main():
    ...
    episode_path = args.input
    work_path = args.work_path
    search_name = args.search_name
    chapter_out_path = args.output
    delete_themes = args.delete_themes
    ...
    if work_path is None:
        work_path = Path(os.path.dirname(episode_path))
        
    if chapter_out_path is None:
        chapter_out_path = episode_path.with_name(episode_path.stem + ".chapters.txt")
       
    t_path = os.path.join(work_path, ".themes")
    
    # Check that there are valid themes if no_download
    if no_download:
        valid = False
    ...
    for theme_file in os.scandir(t_path):
        if ".ogg" in str(theme_file) and len(offset_list) < 4:
            offset1, offset2 = (find_offset(episode_path, Path(theme_file.path), t_path, make_charts))
            if offset1 != None:
                offset_list.append(offset1)
                offset_list.append(offset2)
    
    file_duration = librosa.get_duration(filename=episode_path)
    offset_list.sort()
    if chapter_validator(offset_list, file_duration):  
        generate_chapters(offset_list, file_duration, chapter_out_path)
    else:
        pass
